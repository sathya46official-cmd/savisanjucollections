...
export default function ProductDetailClient({ product, variant, categoryId }: Props) {
  const router = useRouter();

  const allImages = [variant.image_url, ...(variant.additional_images || [])];
  ...
  return (
    <div className="min-h-screen bg-[#F4F2EC] pt-24 pb-16 px-6 md:px-12 lg:px-24">
      ...
      <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24">
          ...
          <div className="flex md:flex-col gap-4 overflow-x-auto md:overflow-y-auto hide-scrollbar w-full md:w-24 flex-shrink-0">
            {allImages.map((img, idx) => (
              <button
                ...
                className={`w-20 h-24 md:w-full md:h-32 flex-shrink-0 border-2 transition-all ${
                  selectedImage === img
        ...
        <div className="flex flex-col py-4">
          <div className="border-b border-[#EAE6D9] pb-8 mb-8">
            <div className="text-xs tracking-[0.3em] text-[#8C8776] uppercase mb-4">
            ...
            <div className="flex items-end gap-4 mb-4">
              ...
              <span className="text-sm tracking-widest text-[#8C8776] uppercase pb-1">
                {variant.is_negotiable && variant.price
                  ? "(Price Negotiable)"
                  : variant.price
                  ? "(Fixed Price)"
                  : ""}
          ...
          <div className="prose prose-sm text-[#5C584E] font-light leading-relaxed mb-10">
            ...
            <p className="mt-4">
              Woven with absolute precision and care, this masterpiece reflects
              true elegance. Ideal for festive occasions and evening soirees,
              bringing a touch of luxury to every drape.
            </p>
          </div>

          <div className="space-y-4 mb-10">
            <h3 className="text-xs font-medium tracking-[0.2em] text-[#1A1A1A] uppercase">
              Color Specification
            </h3>
            <div className="flex items-center gap-3">
              ...
              />
              <span className="text-sm text-[#5C584E] lowercase">
                {variant.color_name}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 border-t border-[#EAE6D9] pt-8">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="text-xs tracking-widest uppercase text-[#1A1A1A] mb-1">
                  Express Delivery
                </h4>
                <p className="text-[10px] text-[#8C8776] uppercase">
      ...
      <CheckoutModal
        ...
        variant={{
          id: variant.id,
          product_id: variant.product_id ?? categoryId,
